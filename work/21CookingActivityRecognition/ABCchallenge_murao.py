...
#        score = self.softmax(y)
        return y ##BCEWithLogitLoss inplements activation function

def ActToIndex(activity, default=False):
    if activity in activities:
        return activities.index(activity)
    else:
        return default
    
def RecpieToIndex(recipe, default=False):
    if recipe in recipes:
        return recipes.index(recipe)
    else:
        return default

def MultiLabelBinarizer(label, NUM_CLASSES):
    y = np.zeros(NUM_CLASSES)
    for i in label:
        y[i] = 1
    return y.tolist()

def MultiLabelDebinarizer(label):
...
def CalcMultiLabelAccuracy(pred, target):
...
for bodypart in bodyparts:
    ...
    #train_x = torch.zeros([len(files), MAX_SEQ_LEN, 3], dtype=torch.float32) #batch(num files) * len sequence(1000) * dim(x,y,z)
    activityset = set()
    num_file = 0
    train_x = []
    train_y = []
    ...
    if is_activity_recognition:
        ...
        if is_multilabel:
            print("multilabel is valid for activity")
            sys.exit()
        elif not is_multilabel:
    ...
    if is_batch:
        for epoch in range(EPOCH_NUM):
            ...
            if (epoch+1) % 100 == 0:
                ...
                if is_multilabel:
                    ...
                    print("epoch:\t{}\ttotal loss:\t{}\tAccuracy:\t{}\ttime:{}".format(epoch+1,total_loss,np.count_nonzero(y_.cpu()==train_y)/len(y_), ed-st))
                st = datetime.datetime.now()
        f = open(bodypart+"_HIDDEN"+str(HIDDEN_SIZE)+"_prediction.txt", "w")
        for i in header:
            f.write(i+"\t")
        f.write("\n")
        for i in range(len(FinalPred[0])):
            ...
            f.write("\n")
        f.close()
        g = open(bodypart+"_HIDDEN"+str(HIDDEN_SIZE)+"_sigmoid.txt", "w")
        for i in header:
            g.write(i+"\t")
        g.write("\n")
