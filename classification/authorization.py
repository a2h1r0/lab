...
## データの計算 ##
# 各データ，各取得回ごとに平均値を計算
vector_ave = np.zeros((len(tester), 14, sensors))   # vector_ave[被験者][取得回数][センサ番号(ベクトル要素)]
for order in range(len(tester)):    ## 被験者ごとに順番に処理
    # 被験者変更時に変数を初期化
    vector_sum = [0 for i in range(sensors)]    # ベクトルの合計
    ...
    for row in data[order].itertuples(name=None):   ## 1行ずつ読み出し
        ...
        if (row[-1] != 0):  # 区切りの検出
            ...
            if (n != 1):    # 最初の区切り"1"ではスキップ
                for i in range(sensors):    ## 平均値を計算(要素ごとに除算していく)
                    vector_ave[order][n-2][i] = vector_sum[i]/num   # 次の要素番号が代入されている状態で保存のため"n-2"
                                                                    # 例: 取得番号"2"の始まりで"1"のデータを[0]に保存
                vector_sum = [0 for i in range(sensors)]    # 変数の初期化
                num = 0
            
        for i in range(sensors):
            # ベクトルのノルム(大きさ)を計算
            vector_sum[i] += row[i+1]    # row[0]にはデータ番号が格納されている
            
        num += 1                            # データ数(計算回数)を増加
    # 最終データの平均値を保存
    for i in range(sensors):
        vector_sum[i] /= num
    vector_ave[order][n-2] = vector_sum    # 区切り文字なしでデータが終了するため
