...
attack_tester = pd.readcsv('attack.csv', usecols = ['Tester']) # 認証データ
train_tester = pd.readcsv('train.csv', usecols = ['Tester'])  # 学習データ
    ...
    for attack in attack_file:              # 認証データ変更ループ
        ...
        for train in train_file:
            ...
            if (distance <= (thresholds/10)):   # ループ用に整数化してあるので÷10
                if (attack_tester[i] == 'Fujii'): # 認証データが正解(本人)
                    TAR[threshold] += 1 # 本人受入数
                else:                                    # 認証データが攻撃(他人)
                    FAR[threshold] += 1 # 他人受入数
            # 距離が閾値より大きいなら，弾く
            elif (distance > (thresholds/10)):
                if (attack_tester[i] == 'Fujii'): # 正解(本人)の場合
