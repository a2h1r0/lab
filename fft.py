...
take_data = np.loadtxt("subject1_file_119.csv", delimiter=",", skiprows=1)
take_x = []
take_y = []
take_z = []
for val in take_data:
    ...
    take_y.append(val[1])
    take_z.append(val[2])
take = take_x + take_y + take_z
take_fft = fft(take)

take_wash_data = np.loadtxt("subject1_file_523.csv", delimiter=",", skiprows=1)  
take_wash_x = []
take_wash_y = []
take_wash_z = []
...
take_wash = take_wash_x + take_wash_y + take_wash_z
take_wash_fft = fft(take_wash)

take_wash2_data = np.loadtxt("subject1_file_491.csv", delimiter=",", skiprows=1)
take_wash2_x = []
take_wash2_y = []
take_wash2_z = []
...
take_wash2 = take_wash2_x + take_wash2_y + take_wash2_z
take_wash2_fft = fft(take_wash2)

take3_data = np.loadtxt("subject1_file_319.csv", delimiter=",", skiprows=1)
take3_x = []
take3_y = []
take3_z = []
for val in take3_data[:len(take_data)]:
    take3_x.append(val[0])
    take3_y.append(val[1])
    take3_z.append(val[2])
take3 = take3_x + take3_y + take3_z
...
"""
distance, path = fastdtw(take2_sum, take3_sum)

print(distance)


plt.plot(take2_sum, label="take2_sum", color="red")
plt.plot(take3_sum, label="take_sum", color="blue")

for take2_sum_x, take3_sum_x in path:
  plt.plot([take2_sum_x, take3_sum_x], [take2_sum[take2_sum_x], take3_sum[take3_sum_x]], color='gray', linestyle='dotted', linewidth=1)

plt.legend()
plt.show()
"""

res = sm.tsa.seasonal_decompose(take_sum, freq=90)
res2 = sm.tsa.seasonal_decompose(take2_sum, freq=90)
res3 = sm.tsa.seasonal_decompose(take3_sum, freq=90)

# 季節成分の可視化
plt.subplots_adjust(hspace=0.3)
plt.figure(figsize=(15, 9))
plt.subplot(411)
plt.plot(res.observed, lw=.6, c='blue')
plt.plot(res2.observed, lw=.6, c='red')
plt.plot(res3.observed, lw=.6, c='green')
plt.title('observed')
plt.subplot(412)
plt.plot(res.trend, lw=.6, c='blue')
plt.plot(res2.trend, lw=.6, c='red')
plt.plot(res3.trend, lw=.6, c='green')
plt.title('trend')
plt.subplot(413)
plt.plot(res.seasonal, lw=.6, c='blue')
plt.plot(res2.seasonal, lw=.6, c='red')
plt.plot(res3.seasonal, lw=.6, c='green')
plt.title('seasonal')
plt.subplot(414)
plt.plot(res.resid, lw=.6, c='blue')
plt.plot(res2.resid, lw=.6, c='red')
plt.plot(res3.resid, lw=.6, c='green')
plt.title('residual')

"""
resid_mat = pd.DataFrame()
for ts in[take_sum, take2_sum, take3_sum]:
    res = sm.tsa.seasonal_decompose(ts, freq=90)
    resid_mat = pd.concat([resid_mat, pd.Series(res.resid)], axis=1)

resid_mat.columns =[['take1_sum', 'take2_sum', 'take3_sum']]
