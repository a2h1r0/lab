...
def make_train_data():
    ...
    for filename in TRAIN_FILES:
        ...
        for index in range(0, len(data) - WINDOW_SIZE + 1, STEP):
            ...
            train_data.append(fft(data[start:end + 1]))
...
def make_test_data():
    ...
    for index in range(0, len(data) - WINDOW_SIZE + 1, STEP):
        ...
        test_data.append(fft(data[start:end + 1]))
...
def main():
    ...
    def train():
        ...
        for epoch in range(EPOCH_NUM):
            ...
            # Tensorへ変換
            inputs = torch.tensor(random_data, dtype=torch.float, device=device).view(-1, 1, random_data[0].size)
    ...
    # Lossの描画
    figures_dir = '../../figures/fft_cnn_' + now
...
if __name__ == '__main__':
    ...
    result_file = '../../data/result_fft_cnn_' + now + '.csv'
