...
def main():
    ...
    def test():
        ...
        with torch.no_grad():
            ...
            predictions_macro.append(outputs_macro.to('cpu').detach().numpy().copy())

            # micro識別
            outputs_micro = model.Micro(inputs, test_data_length)
            predictions_micro.append(outputs_micro.to('cpu').detach().numpy().copy())
