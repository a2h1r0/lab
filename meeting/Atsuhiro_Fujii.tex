...
\subsection{脈波}
Arduinoに接続した脈波センサでピーク検出を行い，画面描画するプログラムを実装しました．
デバイスを今日取りに来たので，明日以降試します．

\subsection{水量推定}
とりあえずConv1Dで実装してみました．5種類の入れ物で6回データを取得．
5回分学習で1回をテスト．誤差10~15\%です．
素の状態でCNNに入れるのとパワースペクトルにするの，どちらがいいのか．
フィルタなどで前処理するべきか．
NNはなにがよいのかなど．
メモリ不足でウィンドウサイズやステップ幅を雑にしているので，もう少し精度が期待できる？
