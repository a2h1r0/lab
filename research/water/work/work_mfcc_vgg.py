...
MFCC_FILTER_NUM = 1024
MFCC_DIMENSION_NUM = 1024
...
def main():
    ...
    # Lossの描画
    figures_dir = '../figures/mfcc_vgg_' + now
...
if __name__ == '__main__':
    ...
    result_file = '../data/result_mfcc_vgg_' + now + '.csv'
    with open(result_file, 'w', newline='') as f:
        result_writer = csv.writer(f)
        result_writer.writerow(['TestFile', 'Diff'])

        # 予測値の保存ファイル作成（検証用）
        log_file = '../data/outputs_mfcc_vgg_' + now + '.csv'
