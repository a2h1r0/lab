...
SAMPLE_SIZE = 2000  # サンプルサイズ
EPOCH_NUM = 1200000  # 学習サイクル数
KERNEL_SIZE = 13  # カーネルサイズ（奇数のみ）

FILE_EPOCH_NUM = 3000  # 1ファイルに保存するエポック数
...
def train():
    ...
    '''モデルの構築'''
    model = Autoencoder(kernel_size=KERNEL_SIZE, device=device)
    ...
    for epoch in range(1, EPOCH_NUM+1):
        ...
        # ---------------------
        # 色データの生成（G(x)）
        fake_colors = model(input_pulse)

        loss = criterion(fake_colors, real_colors)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        #  データの保存
        # ---------------------
        write_data = [epoch, loss.item()]
        print('Loss: {:.3f}'.format(write_data[1]))
